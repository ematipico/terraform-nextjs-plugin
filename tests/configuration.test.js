...
describe("Configuration", () => {
	it("should throw an error when configuration is empty", () => {
		expect(() => checkConfiguration()).toThrowError(
			"Empty configuration, cannot proceed."
		);
	});

	it("should throw an error when gatewayKey is not provided", () => {
		expect(() => checkConfiguration({})).toThrowError(
			"gatewayKey is missing, it must be provided"
		);
	});

	it("should throw an error when gatewayKey is not provided", () => {
		expect(() => checkConfiguration({ gatewayKey: "myTest" })).toThrowError(
			"lambdaPath is missing, it must be provided"
		);
	});

	it("should throw an error when routes is not provided", () => {
		expect(() =>
			checkConfiguration({ gatewayKey: "myTest", lambdaPath: "/path" })
		).toThrowError("routes is missing, it must be provided");
	});
	...
	it("should return the gateway key", () => {
		...
		expect(getGatewayResourceId()).toEqual(
			"${aws_api_gateway_rest_api.myTest.id}"
		);
	});
